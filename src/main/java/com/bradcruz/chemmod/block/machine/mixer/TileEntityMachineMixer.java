...
public class TileEntityMachineMixer extends TileEntity implements IInventory, ITickable {
    ...
    private boolean canSmelt() {
        if(((ItemStack)this.inventory.get(0)).isEmpty() || ((ItemStack)this.inventory.get(1)).isEmpty()) return false;
            ...
            ItemStack result = RecipesMachineMixer.getInstance().getMachineMixerResult((ItemStack)this.inventory.get(0), (ItemStack)this.inventory.get(1));
    ...
    public void smeltItem() {
        if(this.canSmelt()) {
            ...
            ItemStack result = RecipesMachineMixer.getInstance().getMachineMixerResult(input1, input2);
            ItemStack output = (ItemStack)this.inventory.get(3);
    ...
    public void update() {
        ...
        if(isActive) --this.burnTime;

        if(!this.world.isRemote) {
            ItemStack stack = (ItemStack)this.inventory.get(2);

            if(isActive || !stack.isEmpty() && !((((ItemStack)this.inventory.get(0)).isEmpty()) || ((ItemStack)this.inventory.get(1)).isEmpty())) {
            ...
            else if(!isActive && this.cookTime > 0) {
                this.cookTime = MathHelper.clamp(this.cookTime - 2, 0, this.totalCookTime);
            }
            if(this.isActive() != wasActive) {
                flag = true;
                BlockMachineMixer.setState(this.isActive(), this.world, this.pos);
            }
        }
        if(flag)
            this.markDirty();
